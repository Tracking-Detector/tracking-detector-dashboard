<template>
    <div>
        <v-card v-for="job in jobs" :key="job.id" class="mt-3">
            ...
            <v-card-subtitle>{{ job.jobDescription }} <br> {{ job.cronPattern }} <br> <v-chip small class="mt-2" :color="job.enabled ? 'green' : 'red'"> {{ job.enabled ? "Enabled" : "Disabled" }}</v-chip> </v-card-subtitle>
            <v-card-actions>
      <v-btn
